...
export default function RegisterScreen() {
  ...
  const validateForm = () => {
    ...
    else if (!strongPassword(password)) newErrors.password = "Min 12 chars incl. upper, lower, number, symbol";
  ...
  const handleRegister = async () => {
    ...
    if (breached) {
      setErrors((prev) => ({ ...prev, password: "This password appears in breach lists. Choose another." }));
      return;
    }
    try {
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.container} testID="register-screen">
          ...
          <View style={styles.formContainer}>
            ...
            <Input label="Password" placeholder="Create a password" value={password} onChangeText={setPassword} secureTextEntry error={errors.password} testID="password-input" />

            <Input label="Confirm Password" placeholder="Confirm your password" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry error={errors.confirmPassword} testID="confirm-password-input" />

            <Button title="Sign Up" onPress={handleRegister} loading={loading} style={styles.button} testID="register-button" />
